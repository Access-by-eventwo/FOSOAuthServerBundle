...
class OAuthFactory implements SecurityFactoryInterface, AuthenticatorFactoryInterface
{
    ...
    public function createAuthenticator(
        ...
        $managerLocator->replaceArgument(
            0,
            array_merge(
                $managerLocator->getArgument(0),
                [$firewallName => new ServiceClosureArgument(new Reference($managerId))]
            )
        );
        ...
        $container
            ->setDefinition(
                'security.firewall.authenticator.'.$firewallName,
        ...
        $container
            ->setDefinition(
                'security.listener.user_checker.'.$firewallName,
        ...
        if ($container->hasDefinition('security.command.debug_firewall')) {
            ...
            $debugCommand->replaceArgument(
                3,
                array_merge($debugCommand->getArgument(3), [$firewallName => $authenticators])
            );
        }
